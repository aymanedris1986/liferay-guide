...
Use the **`user-chrome-devtools`** MCP (`new_page`, `list_pages`, `navigate_page`, `take_snapshot`, `click`, `fill`, `fill_form`, `evaluate_script`, `take_screenshot`, `close_page`). Discover the tool schemas before the first call. Do **not** use `cursor-ide-browser` for these demos.

Write the ledger and screenshots under `.cursor/skills/liferay-guide-workspace/` (create the folder if needed).
...
- **Keep a ledger**: after each create, append a line to `.cursor/skills/liferay-guide-workspace/liferay-demo-ledger.md` (type, name, id/ERC, URL). Cleanup uses it.
...
- `take_screenshot` at each milestone, with `filePath` under `.cursor/skills/liferay-guide-workspace/` (for example `demo-01-object-published.png`), and mention the files in the report.
